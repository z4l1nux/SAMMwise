...
import type {
    ScoreData, ScorePayload, PreviousPayload, PracticeRow, ScoreBand, AnalysisResult,
} from '../types';
...
const Results = () => {
    ...
    useEffect(() => {
        ...
        for (let dataNum = 0; dataNum < dataENV.length; dataNum++) {
            ...
            const practiceLabels = testCalc.practiceNames.map(
                (name: string) => tCharts(`practices.${name}`) || name
            );
    ...
    const scoreLabel = scoreData ? `${t('overallScore')} ${scoreData.score}/3` : '';
            ...
            <div ref={componentRef}>
                    ...
                    <SectionCard id="totalsDiv" title={t('totalsSection')}>
                        ...
                        <div className="flex justify-center gap-3 flex-wrap mt-4 mb-2">
                            {maturityBands.map(b => (
                                <div
                                    key={b.labelKey}
                                    className="flex items-start gap-2 bg-white/5 rounded-lg p-2.5 px-3.5 border-2 min-w-[160px]"
                                ...
                                >
                                    ...
                                    <div>
                                        <div className="font-bold text-sm text-slate-200">
                                            {t(`maturityBands.${b.labelKey}`)} <span className="font-normal text-slate-500">({b.range})</span>
                                        </div>
                        ...
                        <h3 className="text-center mt-6 mb-3 text-slate-400 font-semibold">{t('responseCount')}</h3>
                    ...
                    <SectionCard id="bussFuncSection" title={t('maturityByBusiness')}>
                        <div className="flex flex-wrap gap-4">
                            <div className="flex-[1_1_45%] min-w-[280px]">
                                <h3 className="text-sm text-slate-400 font-semibold mb-3 uppercase tracking-[0.05em]">{t('maturityByBusinessRadar')}</h3>
                                <Radar data={bussFuncRadar.metaData} options={bussFuncRadar.layout_props} className="bussFuncRadar" />
                            </div>
                            <div className="flex-[1_1_45%] min-w-[280px]">
                                <h3 className="text-sm text-slate-400 font-semibold mb-3 uppercase tracking-[0.05em]">{t('maturityByBusinessBar')}</h3>
                ...
                {display && (
                    <SectionCard id="practiceSection" title={t('maturityByPractice')}>
                        <div className="flex flex-wrap gap-4">
                            <div className="flex-[1_1_45%] min-w-[280px]">
                                <h3 className="text-sm text-slate-400 font-semibold mb-3 uppercase tracking-[0.05em]">{t('maturityByPracticeRadar')}</h3>
                                <Radar data={practiceRadar.metaData} options={practiceRadar.layout_props} className="practiceRadar" />
                            </div>
                            <div className="flex-[1_1_45%] min-w-[280px]">
                                <h3 className="text-sm text-slate-400 font-semibold mb-3 uppercase tracking-[0.05em]">{t('maturityByPracticeBar')}</h3>
