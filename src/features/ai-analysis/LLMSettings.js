...
/**
 * LLMSettings — embeddable settings panel (used in the dedicated AI page).
 * Props:
 *   onSaved(cfg|null) — called after save or clear
 */
export default function LLMSettings({ onSaved }) {
    const t = useTranslations('llm');

    const [provider,     setProvider]     = useState('anthropic');
    const [apiKey,       setApiKey]       = useState('');
    const [apiKeyMasked, setApiKeyMasked] = useState(false); // true when showing placeholder for saved key
    ...
    useEffect(() => {
        ...
        if (stored.model) {
            // Pre-populate saved model; actual list loads when user clicks "Fetch"
    ...
    const handleApiKeyChange = (v) => {
        setApiKey(v);
        setApiKeyMasked(false);
        // Clear models so user must re-fetch after changing key
        setModels([]);
        setModel('');
        setModelError('');
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const payload = { provider, encryptedKey, model, ollamaUrl, autoAnalysis };
    ...
    const rowStyle = { display: 'flex', gap: '10px', alignItems: 'flex-end' };
